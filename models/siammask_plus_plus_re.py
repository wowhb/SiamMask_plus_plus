...
#        self.neck = AdjustAllLayer(**{'in_channels' : [512, 1024, 2048] , 'out_channels' : [256, 256, 256]})
    ...
    def feature_extractor_forward_all(self, x):
        return self.features.forward_all(x)  

    def rpn(self, template, search):
        template_feature = self.feature_extractor(template)
        search_feature = self.feature_extractor(search)
#        template_feature = self.neck(template_feature)
#        search_feature = self.neck(search_feature)        
...
#        template_feature = self.neck(template_feature)
#        search_feature = self.neck(search_feature)    
        pred_mask = self.mask_model(template_feature, search_feature)
        return pred_mask
...
#        search_feature = self.neck(search_feature)        
        rpn_pred_cls, rpn_pred_loc = self.rpn_model(template_feature, search_feature)
        feature = self.feature_extractor_forward_all(search)                
        corr_feature = self.mask_model(template_feature, search_feature)
