...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        if is_valid_number(loss.item()):
            optimizer.step()
        siammask_plus_plus_loss = loss.item()
        batch_time = time.time() - end
        avg.update(batch_time=batch_time, rpn_cls_loss=rpn_cls_loss, rpn_loc_loss=rpn_loc_loss,
                   rpn_mask_loss=rpn_mask_loss, siammask_plus_plus_loss=siammask_plus_plus_loss,
        ...
        if (iter + 1) % args.print_freq == 0:
            logger.info('Epoch: [{0}][{1}/{2}] lr: {lr:.6f}\t{batch_time:s}\t{data_time:s}'
                        '\t{rpn_cls_loss:s}\t{rpn_loc_loss:s}\t{rpn_mask_loss:s}\t{siammask_plus_plus_loss:s}'
                        ...
                        rpn_mask_loss=avg.rpn_mask_loss, siammask_plus_plus_loss=avg.siammask_plus_plus_loss, mask_iou_mean=avg.mask_iou_mean,
